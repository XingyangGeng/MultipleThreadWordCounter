...
public class Counter {
	...
	private static final String USAGE = "Usage: Counter x y z where x is either thread or executor, y is the concurrency-factor and z is the word to count";
	...
	public Counter(Map<String, String> filesToCount, FilesWordCounter counter) {
		this.filesToCount = (filesToCount == null ? Collections
				.<String, String> emptyMap() : new ConcurrentHashMap<>(
				filesToCount));
	...
	/**
	 * @param word
	 *            to count amongst all {@linkplain #filesToCount}
	 */
	public void count(final String word) {
		CountDownLatch latch = new CountDownLatch(filesToCount.size());
		counter.count(filesToCount, word, new FilesWordCounter.Callback() {
			@Override
			public void counted(String fileName, long count) {
				System.out.printf("Word %s appears %d times within %s%n", word,
						count, fileName);
				latch.countDown();
			}
		});
